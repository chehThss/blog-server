...
class Repo:
    ...
    async def init_for_user(self, user):
        makedirs(path.join(self.upload_path, 'private', user['id']), exist_ok=True)

    async def del_for_user(self, user):
        rmtree(path.join(self.upload_path, 'private', user['id']))
    ...
    @staticmethod
    def send(p, allow_dir=False):
        if path.isdir(p) and allow_dir:
            result = {}
            for k in listdir(p):
                if not k.startswith('.'):
